...
$(document).ready(function () {
    ...
    function initContainerListES() {
        if (containerListSource == null || containerListSource.readyState == 2) {
            containerListSource = new EventSource(`https://${window.location.hostname}/api/streams/containerlist`);
    ...
    function initMessageES() {
        if (messagesSource == null || messagesSource.readyState == 2) {
            messagesSource = new EventSource(`https://${window.location.hostname}/api/streams/servermessages`);
    ...
    function initImageListES() {
        if (imageListSource == null || imageListSource.readyState == 2) {
            imageListSource = new EventSource(`https://${window.location.hostname}/api/streams/imagelist`);
    ...
    function initContainerStatsES() {
        if (containerStatsSource == null || containerStatsSource.readyState == 2) {
            ...
            containerStatsSource = new EventSource(`https://${window.location.hostname}/api/streams/containermetrics`);//how come /containermetrics doesnt have the id
    ...
    $('#upload-compose-btn').click(function (e) {
        ...
        $.ajax({
            url: `https://${window.location.hostname}/api/compose/upload`,
            type: 'POST',
            data: JSON.stringify({
    ...
    function initComposeFilesSource() {
        if (composeFilesSource == null || composeFilesSource.readyState == 2) {
            composeFilesSource = new EventSource(`https://${window.location.hostname}/api/streams/composefiles`);
    ...
    $('#create-container-btn').on('click', function () {
        ...
        // ajax request
        $.ajax({
            url: `https://${window.location.hostname}/api/containers/run`,
            type: 'post',
    ...
    $('#confirm-prune').on('click', function () {
        ...
        $.ajax({
            url: `https://${window.location.hostname}/api/system/prune`,
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
